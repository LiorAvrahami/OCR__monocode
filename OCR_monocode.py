...
argparser = argparse.ArgumentParser(
    description="OCR program, that reads text from image and exports to text file.\nsee https://github.com/LiorAvrahami/OCR__monocode")
argparser.add_argument("--exportfilesystem", action='store', nargs='?', default="false", const="true", choices=["true", "false"],
                       help="if this atribute is added then no OCR will be used, instead, the selected input file or folder that represents the root of a file "
                            "system, will be converted to a single .txt file that can be printed, and can later be scanned and mapped back to the origional "
                            "file system hyrarchy")
argparser.add_argument("--addFontWithCalibrationFile", action='store', nargs='?', default="false", const="true", choices=["true", "false"],
                       help="if this atribute is added then the selected input file will be used as a calibration file for a new font entry. this new font "
                            "entry will be avalible later for reading other scans that were printted and scanned with similar settings. ")
argparser.add_argument("--createNewCalibrationFile", action='store', nargs='?', default="false", const="true", choices=["true", "false"],
                       help="if this atribute is added then a new calibration text file will be created and stored in the selected output path. this file can "
                            "be printted, then scanned and then passed to this program with \"addFontWithCalibrationFile\" turned on in order to add this "
                            "font settings.")
...
argparser.add_argument("-f", "--fontselect", action='store', nargs=3, default=None,
                       help="select font specifics [font name] [font size] [scanner resolution] dpi\n   example:\n      python OCR_monocode.py -fs Consols 8 "
                            "600dpi")
argparser.add_argument("-n", "--hasnoframe", action='store', nargs='?', default="true", const="false", dest="frame",
                       help="by default the printted text should be surround with a frame of X, to increase percision. if using \"--exportfilesystem\" then "
                            "the correct frame will be automatically added to the text files. this frame will not be in the final .txt files.")
...
argparser.add_argument("-r", "--viewpastresults", action='store', nargs='?', default="false", const="true", choices=["true", "false"],
                       help="if true, only loads ocr_crosscorrelation_results.npz files from past runs, and skips files that don't have ocr result files")
...
user_arguments.viewpastresults = str_to_bool(user_arguments.viewpastresults)
user_arguments.addFontWithCalibrationFile = str_to_bool(user_arguments.addFontWithCalibrationFile)
user_arguments.createNewCalibrationFile = str_to_bool(user_arguments.createNewCalibrationFile)
...
if user_arguments.exportfilesystem:
    ...
    code_to_print.file_tree_to_text_file(input_path, output_path, b_has_frame=bool(user_arguments.frame))
    quit(0)

if user_arguments.addFontWithCalibrationFile:
    from cross_correlation_letter_detector.calibrate_new_font_database.calibration_handler import add_new_font_with_calibration_file

    add_new_font_with_calibration_file(input_path)
    quit(0)
if user_arguments.createNewCalibrationFile:
    from cross_correlation_letter_detector.calibrate_new_font_database.calibration_handler import create_new_calibration_file

    create_new_calibration_file(output_path)
    quit(0)
...
def process_image_to_text(path):
    ...
    if ocr_Mechanism == "ccr":
        out_strings[path] = process_image_to_text_with_ccr(path, selected_font_settings, b_has_frame=bool(user_arguments.frame),
                                                           b_skip_ocr=bool(user_arguments.viewpastresults))
